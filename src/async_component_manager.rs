...
impl<State> AsyncComponentData<State>
where
    State: Clone,
{
    /// Constructor
    #[allow(dead_code)]
    pub fn new(id: ComponentId, initial_state: State) -> AsyncComponentData<State> {
    ...
    pub fn save_message(&mut self, message: Message) -> Result<(), Failure> {
        if message.from != self.id && message.to != self.id || message.is_anti {
...
mod test {
    ...
    fn get_manager() -> AsyncComponentData<i32> {
        AsyncComponentData {
            id: ComponentId {
                federate_id: 1,
                federation_id: 11,
    ...
    fn new_creates_and_takes_a_checkpoint() {
        ...
        let manager = AsyncComponentData::new(id.clone(), initial_state.clone());
    ...
    fn takecheckpoint_adds_a_checkpoint_and_increments_lvt() {
        fn test(a: AsyncComponentData<i32>) {
            let mut b = a.clone();
            b.take_checkpoint();
